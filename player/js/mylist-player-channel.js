...
for (var n = 0; n < numPlaylists; n++) {
  ...
  videosURL[n] =
    ...
    '&key=' +
    apiKey +
    '&fields=items&part=snippet&maxResults=50&vq=hd1080';
}
...
function doAjaxCallStuff(n) {
  ...
  $.ajax({
    url: 'https://api.rss2json.com/v1/api.json?rss_url=https://www.youtube.com/feeds/videos.xml?channel_id=UCMYzTKe3BBnL0OOoOXhEzKQ',
    dataType: 'json',
    async: true,
    success: function (data) {
      ...
      $.each(data.items, function (i, val) {
        ...
        if (i === 0) {
          // so we can set first vid as current
          // if(isMobile.iOS()) { // if iOS use cueVideoById initially
          if (myVar === 1) {
            // if iOS use cueVideoById initially
            list_data +=
              '<div class="vid-item" tabindex="0" onClick="vid_frame[' +
              m +
              ...
              '; setCurrent(' +
              m +
              ')" onfocus="$(document).keyup(function(e) { if(e.keyCode ==13) { vid_frame[' +
              m +
          ...
          list_data +=
            '<div class="thumb"><img class="current-vid" src="' +
            thumb +
            '"></div>\n';
          if (showTitlesInList === 1) {
            list_data +=
              '<div class="desc current-vid">' + feedTitle + '</div>\n';
          }
          list_data += '</div>';
        } else {
          if (myVar === 1) {
      ...
      $(
        '#player-container' + m + '>div.mlvp-list-container>div.mlvp-list'
      ).width(list_width[m] + 'px');
  ...
  if (autoPlayNext === 1) {
    ...
    if (event.data === 0) {
      ...
      if (nextVid >= listLength[y]) {
        ...
        var xPos = getOffset(
          jQuery('#player-container' + y + '>div.mlvp-list-container')
        ).left;
      ...
      for (var i = 0; i < listLength[n]; i++) {
        ...
        if (
          $(
            '#player-container' +
              y +
              ' .mlvp-list-container .mlvp-list .vid-item:first-child'
          )
            .attr('onClick')
            .indexOf('cueVideoById') > -1
        ) {
          $(
            '#player-container' +
              y +
              '>div.mlvp-list-container>div.mlvp-list.vid-item:nth-child(' +
              lnum +
              ')'
          ).attr('onclick', '');
        }
...
function setCurrent(n) {
  $(
    '#player-container' +
      n +
      '>div.mlvp-list-container>div.mlvp-list>div.vid-item>div.current-vid'
  ).removeClass('current-vid');
  $(
    '#player-container' +
      n +
      '>div.mlvp-list-container>div.mlvp-list>div.vid-item>div.thumb img.current-vid'
  ).removeClass('current-vid');
  ...
  $(
    '#player-container' +
      n +
      '>div.mlvp-list-container>div.mlvp-list div:nth-child(' +
      currNum +
      ') div.desc'
  ).addClass('current-vid');
  $(
    '#player-container' +
      n +
      '>div.mlvp-list-container>div.mlvp-list div:nth-child(' +
...
function resetOnClick() {
  ...
  for (var i = 0; i < listLength[y]; i++) {
    ...
    $(
      '#player-container' +
        y +
        '>div.mlvp-list-container>div.mlvp-list>div.vid-item:nth-child(' +
        lnum +
        ')'
    ).attr('onclick', clickString);
  }
  return;
}
...
function setScrollAmt(n) {
  ...
  jQuery(
    '#player-container' + m + '>div.mlvp-arrows>div.mlvp-arrow-right'
  ).bind('click', function (event) {
    event.preventDefault();
    jQuery('#player-container' + m + '>div.mlvp-list-container')
      .stop()
      .animate(
        {
  ...
  jQuery('#player-container' + m + '>div.mlvp-arrows>div.mlvp-arrow-left').bind(
    'click',
    function (event) {
      event.preventDefault();
      jQuery('#player-container' + m + '>div.mlvp-list-container')
...
function scrollListLeft(m) {
  // event.preventDefault();
  $('#player-container' + m + '>div.mlvp-list-container')
    .stop()
    .animate(
      {
        scrollLeft: '+=' + scrollAmt,
